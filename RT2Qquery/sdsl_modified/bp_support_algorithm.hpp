...
/*
    uint64_t near_rmq_leftnew(const bit_vector& bp, uint64_t l, uint64_t r,bit_vector::difference_type& min_rel_ex)
    {
        typedef bit_vector::difference_type difference_type;
        difference_type excess = 0;
        difference_type min_pos=l;
        min_rel_ex = 0;
        for (uint64_t j=l+1; j < r+1; ++j) {
            if (bp[j])
                ++excess;
            else {
                --excess;
