...
class bp_support_sada
{
    ...
    //private:
        ...
        std::vector<uint8_t> min_pos_min;
...
#define USE_CACHE
        ...
        void copy(const bp_support_sada& bp_support)
        {
            ...
            min_pos_min.resize(bp_support.min_pos_min.size());
            std::copy(bp_support.min_pos_min.begin(),bp_support.min_pos_min.end(),min_pos_min.begin());
        ...
        {
            return m_size-((difference_type)m_med_block_min_max[v]);
        }
        ...
        {
            return (1 - ((difference_type)m_sml_block_min_max[sml_block]));
        }
        ...
        size_type bwd_excess_in_med_block(size_type sml_block_idx, difference_type desired_excess)const
        {
            ...
            while ((sml_block_idx+1) and sml_block_idx >= first_sml_block_in_med_block) {
                ...
                difference_type min_ex     = ex + (1 - ((difference_type)m_sml_block_min_max[sml_block_idx]));
    ...
    public:
            ...
            m_med_inner_blocks(0)
        {
            ...
            m_sml_block_min_max = int_vector<>(m_sml_blocks, 0, bits::hi(t_sml_blk+2)+1);
            m_med_block_min_max = int_vector<>((m_med_blocks+m_med_inner_blocks), 0, bits::hi(2*m_size+2)+1);
            ...
            for (size_type i=0; i < m_size; ++i) {
                ...
                if ((i+1)%t_sml_blk == 0 or i+1 == m_size) {
                    ...
                    m_sml_block_min_max[sidx    ] = -(min_ex-1);
                    size_type v = m_med_inner_blocks + sidx/t_med_deg;

                    if ((difference_type)(-(curr_abs_ex + min_ex)+m_size) > ((difference_type)m_med_block_min_max[v])) {
                        assert(curr_abs_ex+min_ex <= min_value(v));
                        m_med_block_min_max[v] = -(curr_abs_ex + min_ex)+m_size;
                    }
            ...
            for (size_type v = m_med_block_min_max.size() - 1; !is_root(v); --v) {
                size_type p = parent(v);
                if (min_value(v) < min_value(p))  // update minimum
                    m_med_block_min_max[p] = m_med_block_min_max[v];
            }

            //calculate min_pos_min
            min_pos_min = std::vector<uint8_t>(256,0);
        ...
        //size_type select_without_substructure(size_type i)const//binary search select
        size_type select_bin(size_type i)const
        {
            int l = 0;
            int r = m_size-1;
            int mid = (l + r)/2;
            ...
            for(result = mid;result>=0 && rank(result) == rank(mid);result--);
            return result+1;
        }

        size_type select0_bin(size_type i)const
        {
#ifdef USE_CACHE
            size_type a = 0;
            ...
            if(2*idx-OPT_SIZE > 0 and 2*idx-OPT_SIZE < m_size){
                if(rank0(2*idx-OPT_SIZE) < i)
                    l = 2*idx-(OPT_SIZE-1);
            }
            if(2*idx+OPT_SIZE < m_size){
                if(rank0(2*idx+OPT_SIZE) > i)
                    r = 2*idx+OPT_SIZE-1;
            }
            ...
            for(result = mid;result>=0 and (*m_bp)[result]==1;result--);
#ifdef USE_CACHE
            select_cache.write(i, result);
#endif
            return result;
        }
        void initializeCache()const {
            ...
            for(int i=0;i<CACHE_SIZE+1;i++) {
                select_cache.m_table[i<<1] = (size_type)-1;
                find_open_cache.m_table[i<<1] = (size_type)-1;
                find_close_cache.m_table[i<<1] = (size_type)-1;
            }
#endif
        }
        ...
        size_type median_block_rmq_left(size_type l_sblock, size_type r_sblock, bit_vector::difference_type& min_ex)const
        {
            assert(l_sblock <= r_sblock+1);
            size_type pos_min_block = (size_type)-1;
            difference_type e = 0;
        ...
        size_type min_excess(size_type l, size_type r)const
        {
            assert(l<=r);
            ...
            if (sbl == sbr) { // if l and r are in the same small block
                return excess(near_rmq(*m_bp, l, r, min_rel_ex));
            } else {
                ...
                //std::cout<<"(min_ex : "<<min_ex<<")"<<std::endl;//DEBUG
                ...
                if ((excess(sbr*t_sml_blk) + min_rel_ex) <= min_ex) {             // if it contains the minimum return its position
                    ...
                    return excess(sbr*t_sml_blk) + min_rel_ex;
                }
        ...
        size_type near_rmq_leftnew(size_type l, size_type r,bit_vector::difference_type& min_rel_ex)
        {
            ...
            const uint64_t* b = (*m_bp).data();
            for (uint64_t j=l8; j < r8; j+=8) {
                int8_t x = excess::data.min[(((*(b+(j>>6)))>>(j&0x3F))&0xFF)];
                if ((excess+x) < min_rel_ex) {
                    min_rel_ex = excess+x;
                    //min_pos    = j + excess::data.min_pos_min[(((*(b+(j>>6)))>>(j&0x3F))&0xFF)];
                    min_pos    = j + min_pos_min[(((*(b+(j>>6)))>>(j&0x3F))&0xFF)];
                }
        ...
        size_type rmq_left(size_type l, size_type r)
        {
            assert(l<=r);
            size_type sbl = sml_block_idx(l);
            size_type sbr = sml_block_idx(r);
            ...
            if (sbl == sbr) { // if l and r are in the same small block
                ...
                return near_rmq_leftnew(l, r, min_rel_ex);
            } else {
                difference_type min_ex  = 0;          // current minimal excess value
                size_type        min_pos = 0;          // current min pos
                enum min_pos_type {POS, SMALL_BLOCK_POS, MEDIUM_BLOCK_POS};
                enum min_pos_type pos_type = POS;     // current
                min_pos = near_rmq_leftnew(l, (sbl+1)*t_sml_blk-1, min_rel_ex); // scan the leftmost small block of l
                assert(min_pos >= l);
                min_ex = excess(l) + min_rel_ex;
                ...
                size_type temp = median_block_rmq_left(sbl+1, std::min((mbl+1)*t_med_deg-1, sbr-1), min_ex); // scan the medium block of l
                if (temp != (size_type)-1) {
                    assert(temp* t_sml_blk >= l and temp* t_sml_blk <= r);
                    min_pos     = temp;
                    assert(min_pos >= 0  and min_pos < m_sml_blocks);
                ...
                if (mbr-mbl > 1) {
                    ...
                    if (pos_type == MEDIUM_BLOCK_POS) {
                        while (!is_leaf(min_pos)) {
                            min_pos = left_child(min_pos);
                            if (node_exists(right_sibling(min_pos)) and min_value(min_pos) > min_ex)
                                min_pos = right_sibling(min_pos);
                        }
                    }
                }
#endif
                ...
                // search in the medium block of r
                temp = median_block_rmq_left(std::max(mbr*t_med_deg, sbl+1), sbr-1, min_ex);  // scan the medium block of r
                if (temp != (size_type)-1) {
                    assert(temp* t_sml_blk >= l and temp* t_sml_blk <= r);
                    min_pos     = temp;
                    pos_type     = SMALL_BLOCK_POS;
                ...
                // search in the small block of r
                temp = near_rmq_leftnew(sbr*t_sml_blk, r, min_rel_ex); // scan the small block of r
                if ((excess(sbr*t_sml_blk) + min_rel_ex) < min_ex) {             // if it contains the minimum return its position
                    assert(temp>=l and temp<=r);
                    return temp;
                }
                ...
                if (pos_type == MEDIUM_BLOCK_POS) {
                    min_pos = min_pos - m_med_inner_blocks;
                    temp = median_block_rmq_left(min_pos*t_med_deg, (min_pos+1)*t_med_deg-1, min_ex);
                    assert(temp != (size_type)-1);   // assert that we find a solution
                    assert(temp* t_sml_blk >= l and temp* t_sml_blk <= r);
                    min_pos = temp;
                    pos_type = SMALL_BLOCK_POS;
                ...
                if (pos_type == SMALL_BLOCK_POS) {
                    min_pos = near_rmq_leftnew(min_pos*t_sml_blk, (min_pos+1)*t_sml_blk-1, min_rel_ex);
                    assert(min_pos >=l and min_pos <= r);
                }
                return min_pos;
            }
