...
int main(int argc,char** argv)
{
    ...
    if(stoi(argv[1]) == 0){//Our RMQ TEST MODE
        ...
        ourTest(argv[2],usingSpace,stoi(argv[3]));
    }else if(stoi(argv[1])==2){//TRIVIAL DFUDS TEST MODE
        if(argc < 4){
            cout << "USAGE : ./test MODE PATH QUERYSIZE" << endl;
            exit(0);
        }
        Top2FromRMQTest<DFUDSIndexing>(argv[2],stoi(argv[3]));
...
void printStat(string path,int querySize){
    vector<uint> A = readArrayFromFile(path);
    ofstream depthcsv;
...
void ourTest(string path,double usingSpace,int querySize){
    ...
    //cout<<"depth container size : "<<r2mq_rev->depthValuesContainer.getSize()<<endl;
    reverse(A.begin(),A.end());*/
    DFUDSEncoding* r2mq;
    r2mq = new DFUDSEncoding(A,usingSpace,false);
    //cout<<"depth container size : "<<r2mq->depthValuesContainer.getSize()<<endl;
    ofstream mycsv;
    mycsv.open("data.csv",ios::app);

    //if(r2mq->interval < r2mq_rev->interval) {
