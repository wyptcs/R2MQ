...
public:
    ...
    int lDepthVarSize;
public:
    GetInterval(vector<uint>& array,int depthVarSize,int lDepthVarSize,int querySize)
    {
    ...
    void makeRmqDepthStat(vector<uint>& A,int querySize,int maxDepth){
        vector<int> rmqDepthStat(maxDepth+1,0);
        int iternum = 1000000;
        for(int i=0;i<iternum;i++) {
            int startIndex = rand() % (A.size() - querySize - 1);
            int endIndex = startIndex + querySize - 1;
            uint rmqDepth = r2mq->bps.excess(r2mq->bps.rmq(r2mq->bps.select(startIndex + 2) - 1, r2mq->bps.select(endIndex + 2))) - 1;
