...
template<>
struct rank_support_trait<110,3> {
    typedef rank_support::size_type	size_type;

    static size_type args_in_the_word(uint64_t w, uint64_t& carry)
    {
        //uint64_t carry2 = (w >> 63);
        size_type res = bits::cnt(~w & (w<<1 | carry >> 1) & (w<<2 | carry));
        carry = (w >> 62);
        return res;
    }
    ...
    static uint32_t word_rank(const uint64_t* data, size_type idx)
    {
        ...
        int array[64];
        for(int i=0;i<64;i++)
            array[i] = (~(*data<<2) - 3>>i) & 0x1;
            //array[i] = (test>>i) & 0x1;
        /*
        for(int idx = 1;idx<23;idx++)
            std::cout<<bits::cnt(*data & *data<<1 & ~(*data)<<2 & bits::lo_set[idx&0x3F]);
        std::cout<<std::endl;
        */
        /*
        for(int i=63;i>=0;i--)
...
*/
        ...
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 0;
        uint64_t carry2 = (idx>63) ? *(data-1)>>62 : 0;
        //std::cout<<*data<<std::endl;
        //std::cout<<bits::cnt((*data & ((*data)<<1 | carry) & ~((*data)<<2 | carry2)) & bits::lo_set[idx&0x1F])<<" "<<std::endl;
        return bits::cnt((~(*data) & ((*data)<<1 | carry) & ((*data)<<2 | carry2)) & bits::lo_set[idx&0x3F]);

        //return bits::cnt((~(*data) & (*data)<<1 & (*data)<<2) & bits::lo_set[idx&0x3F]);

        //return bits::cnt(*data & (*data)<<1 & bits::lo_set[idx&0x3F]);
    }
    ...
    static uint32_t full_word_rank(const uint64_t* data, size_type idx)
    {
        ...
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 0;
        uint64_t carry2 = (idx>63) ? *(data-1)>>62 : 0;
        //std::cout<<bits::cnt(*data & ((*data)<<1 | carry) & ~((*data)<<2 | carry2))<<" "<<std::endl;
        return bits::cnt((~(*data) & ((*data)<<1 | carry) & ((*data)<<2 | carry2)));
    ...
    template<>
    struct rank_support_trait<100,3> {
        typedef rank_support::size_type	size_type;
    ...
    */
        ...
        static uint32_t full_word_rank(const uint64_t* data, size_type idx)
        {
            data = data+(idx>>6);
            uint64_t carry = (idx>63) ? *(data-1)>>63 : 0;
            uint64_t carry2 = (idx>62) ? *(data-1)>>62 : 1;
            //std::cout<<bits::cnt(*data & ((*data)<<1 | carry) & ~((*data)<<2 | carry2))<<" "<<std::endl;
            return bits::cnt((~((*data) | ((*data)<<1 | carry)) & ((*data)<<2 | carry2)));
